function obj = Measure( obj, p_s, TARGET, LANDMARK, current_time )
...
for iTarget = 1 : length(TARGET)
    if obj.bTrack(iTarget) == 1
        ...
        p_x = TARGET(iTarget).DYNAMICS.GetPosition();
    
        obj.meas(idxTarget).id = TARGET(iTarget).id;
        obj.meas(idxTarget).y(1,1) = p_x(1) + obj.v(1); %relative easting + bias
        obj.meas(idxTarget).y(2,1) = p_x(2) + obj.v(2); %relative northing + bias
    end
