function [ x_next, F_next, Gamma_next, Gu_next ] = StateUpdate( obj, x_k, u_k, w_k, CLOCK )
%STATEUPDATE generates state with respect to delta_t in CLOCK. this
%function also uses the noise in Dynamics class as well as the current
%state as the initial state

% TO BE IMPLEMENTED:
% switch(option)
%     case('full')
%         % vector with initial conditions: state, state TM, process noise TM, control input TM
%         IC = [ x_k; reshape(eye(3),3*3,1); reshape(zeros(3),3*3,1); reshape(zeros(3,2),3*2,1)];
%     case('state')
%         % vector with initial conditions: state
%         IC = x_k;
% end

switch(obj.spec)
    case('Linear')
        IC = [ x_k; reshape(eye(4),4*4,1); reshape(zeros(4),4*4,1); reshape(zeros(4,2),4*2,1)];
    case('Dubins')
        IC = [ x_k; reshape(eye(3),3*3,1); reshape(zeros(3),3*3,1); reshape(zeros(3,2),3*2,1)];
end

% time vector
TimeProfile = 0:CLOCK.dt:CLOCK.dt;

% solution using ODE45
[~, xout] = ode45(@(t,x)obj.StateDerivative(t,x,u_k,w_k), TimeProfile, IC, obj.ODEoption);

switch(obj.spec)
    case('Linear')
        x_next = xout(end,1:4)';
        F_next = reshape(xout(end,5:20),4,4);
        Gamma_next = reshape(xout(end,21:36),4,4);
        Gu_next = reshape(xout(end,37:end),4,2);
    case('Dubins')
        x_next = xout(end,1:3)';
        F_next = reshape(xout(end,4:12),3,3);
        Gamma_next = reshape(xout(end,13:21),3,3);
        Gu_next = reshape(xout(end,22:end),3,2);
end

end