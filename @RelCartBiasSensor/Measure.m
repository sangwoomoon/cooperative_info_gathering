...
for iTarget = 1 : length(TARGET)
    ...
    obj.meas(iTarget).y(1,1) = obj.bias(1) + delx(1) + obj.v(1); %relative easting + bias
    obj.meas(iTarget).y(2,1) = obj.bias(2) + delx(2) + obj.v(2); %relative northing + bias
...
for iLandmark = 1 : length(LANDMARK)
    ...
    obj.meas(length(TARGET)+iLandmark).y(2,1) = obj.bias(2) + obj.v(2); % bias + noise
end
